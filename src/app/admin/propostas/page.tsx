    ...
    <div className="list-container">
      <div className="section-header">
        <h1 className="title" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <CheckCircle2 size={28} color="#10b981" /> Propostas Enviadas
        </h1>
      ...
      <div className="card filters-card" style={{ padding: '1rem 1.5rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
        ...
        <div className="stats-indicator" style={{ fontSize: '0.875rem', color: '#8d9596' }}>
          {filteredCases.length} propostas ativas
        </div>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} style={{ textAlign: 'center', padding: '3rem' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', color: '#8d9596' }}>
                    ...
                    <p>Nenhuma proposta enviada recentemente.</p>
                  </div>
                </td>
              </tr>
              ...
              <tr key={item.id}>
                ...
                <td>
                  <div className="value-cell" style={{ fontWeight: 700 }}>
                    R$ {(item.estimated_value || 0).toLocaleString('pt-BR')}
                  </div>
                </td>
                <td>
                   <span className="badge-status" style={{ background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', padding: '4px 10px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>
                      ENVIADA
                    </span>
                </td>
                <td>
      ...
      <style jsx>{`
        ...
        .lead-cell strong { display: block; }
      `}</style>
    </div>
  );
}
